...
class gpload:
    ...
    def __init__(self,argv):
        ...
        self.staging_table_name = self.staging_table.replace("'",'')
            ...
            try:
                try:
                    if argv[0]=='-h':
                        ...
                        sys.stderr.write("gpload version 7.0.0 build commit:0a7a3566873325aca1789ae6f818c80f17a9402d\n")
                        sys.exit(0)
                    elif argv[0]=='-v':
        ...
        try:
            ...
            logfile_old = self.options.l
            logfile_new = ''
            count =  0
    
            for i in logfile_old[logfile_old.rfind('/')-1::]:
                if i == '_':
                    count += 1
                    if count == 5:
                        logfile_new += i + "'"
                        continue
                    elif count == logfile_old[logfile_old.rfind('/')-1::].count('_') - 1:
                        logfile_new += "'" + i
                        continue
                logfile_new += i
            logfile_old = logfile_new
            
            self.options.l = logfile_old
            
            
            self.logfile = open(self.options.l,'a')
        except Exception as e:
            self.log(self.ERROR, "could not open logfile %s: %s" % \
                      ("/ipc/Informatica/infa_shared/Temp/debug.txt", e))
            self.log(self.ERROR, "could not open logfile %s: %s" % \
                      (self.options.l, e))
        ...
        try:
            ...
            for i in configFilename[configFilename.rfind('/')-1::]:
                if i == '_':
                    count += 1
                    if count == 5:
                        configFilename_new += i + "'"
                        continue
                    elif count == configFilename[configFilename.rfind('/')-1::].count('_') - 1:
                        configFilename_new += "'" + i
                        continue
                configFilename_new += i
            configFilename = configFilename_new
    ...
    def get_schematable(self, schemaName, tableName):
        ...
        if schemaName is None:
            ...
            schemaTable = schemaTable.replace("'",'')
            return schemaTable
        ...
        if self.reuse_tables == True:
            ...
            with self.conn.cursor() as cur:
                ...
                if len(resultList) > 0:
                    ...
                    self.staging_table_name = self.staging_table_name.replace("'",'')
